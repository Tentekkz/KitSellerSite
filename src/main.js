...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- 12. Kaspi Fee & Net Margin Calculator Engine ---
  const calcCategory = document.getElementById('calc-category');
  const calcSellPrice = document.getElementById('calc-sell-price');
  const calcCostPrice = document.getElementById('calc-cost-price');
  const btnDelivKaspi = document.getElementById('btn-deliv-kaspi');
  const btnDelivPickup = document.getElementById('btn-deliv-pickup');

  const resFeePct = document.getElementById('res-fee-pct');
  const resFeeVal = document.getElementById('res-fee-val');
  const resDelivVal = document.getElementById('res-deliv-val');
  ...
  let currentDeliveryCost = 650;

  if (btnDelivKaspi && btnDelivPickup) {
    btnDelivKaspi.addEventListener('click', () => {
      btnDelivKaspi.classList.add('active');
      btnDelivPickup.classList.remove('active');
      currentDeliveryCost = 650;
      updateKaspiCalculator();
    });

    btnDelivPickup.addEventListener('click', () => {
      btnDelivPickup.classList.add('active');
      btnDelivKaspi.classList.remove('active');
      currentDeliveryCost = 0;
      updateKaspiCalculator();
    });
  }

  const updateKaspiCalculator = () => {
    if (!calcCategory || !calcSellPrice || !calcCostPrice) return;

    const feePct = parseFloat(calcCategory.value) || 10;
    const sellPrice = parseFloat(calcSellPrice.value) || 0;
    const costPrice = parseFloat(calcCostPrice.value) || 0;

    const feeVal = Math.round(sellPrice * (feePct / 100));
    const taxVal = Math.round(sellPrice * 0.03); // 3% ИП налог
    const netProfit = Math.round(sellPrice - costPrice - feeVal - currentDeliveryCost - taxVal);
    const netMargin = sellPrice > 0 ? ((netProfit / sellPrice) * 100).toFixed(1) : 0;

    if (resFeePct) resFeePct.textContent = `${feePct}%`;
    if (resFeeVal) resFeeVal.textContent = `-${feeVal.toLocaleString('ru-RU')} ₸`;
    if (resDelivVal) resDelivVal.textContent = `-${currentDeliveryCost.toLocaleString('ru-RU')} ₸`;
    if (resTaxVal) resTaxVal.textContent = `-${taxVal.toLocaleString('ru-RU')} ₸`;
  ...
  if (calcCategory) calcCategory.addEventListener('change', updateKaspiCalculator);
  if (calcSellPrice) calcSellPrice.addEventListener('input', updateKaspiCalculator);
  if (calcCostPrice) calcCostPrice.addEventListener('input', updateKaspiCalculator);
  updateKaspiCalculator();

  // --- 13. Kaspi Juma Readiness Checklist ---
  const jumaCheckboxes = document.querySelectorAll('.juma-cb');
  const jumaProgPct = document.getElementById('juma-prog-pct');
  const jumaProgFill = document.getElementById('juma-prog-fill');
  const jumaStatusBadge = document.getElementById('juma-status-badge');

  const updateJumaReadiness = () => {
    if (!jumaCheckboxes.length) return;
    let checkedCount = 0;
    jumaCheckboxes.forEach(cb => {
      if (cb.checked) checkedCount++;
    });

    const pct = Math.round((checkedCount / jumaCheckboxes.length) * 100);
    if (jumaProgPct) jumaProgPct.textContent = `${pct}%`;
    if (jumaProgFill) jumaProgFill.style.width = `${pct}%`;
  ...
  jumaCheckboxes.forEach(cb => cb.addEventListener('change', updateJumaReadiness));
  updateJumaReadiness();
